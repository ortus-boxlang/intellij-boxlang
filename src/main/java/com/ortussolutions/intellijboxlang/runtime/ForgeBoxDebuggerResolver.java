...
public final class ForgeBoxDebuggerResolver {

	private static final String ENDPOINT = "https://forgebox.io/api/v1/entry/bx-debugger";

	private ForgeBoxDebuggerResolver() {
	}

	public static ForgeBoxDebuggerDescriptor resolve( String requestedVersion ) throws IOException {
		...
		ForgeBoxDebuggerDescriptor matched = resolveVersion( data, normalized );
		if ( matched != null ) {
			matched.version = normalized;
			return matched;
		}
		throw new IOException( "Unable to resolve bx-debugger download for version: " + normalized );
	}
	...
	private static ForgeBoxDebuggerDescriptor resolveLatest( JsonObject data ) throws IOException {
		JsonObject	latest		= getObject( data, "latestVersion" );
		String		downloadUrl	= getString( latest, "downloadURL" );
		if ( downloadUrl == null ) {
			throw new IOException( "Unable to locate latest bx-debugger download URL." );
		}
		ForgeBoxDebuggerDescriptor descriptor = new ForgeBoxDebuggerDescriptor();
		descriptor.downloadUrl	= downloadUrl;
		descriptor.version		= getString( latest, "version" );
		return descriptor;
	}

	private static ForgeBoxDebuggerDescriptor resolveVersion( JsonObject data, String version ) {
